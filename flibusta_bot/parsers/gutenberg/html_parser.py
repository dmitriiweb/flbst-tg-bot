...
def parse_books(html: str) -> list[schemas.BookListingData]:
    ...
    for book in books:
        ...
        result.append(schemas.BookListingData(
            title=title,
            author=author,
            url=url
        ))
...
def parse_book_info(html: str) -> schemas.BookInfoData | None:
    ...
    author_elements = tree.xpath("//div[@class='breadcrumbs']//a[contains(@href, '/ebooks/author/')]")
    author = ""
    if author_elements:
        ...
        author_elements = tree.xpath("//table[@id='about_book_table']//tr[th[text()='Author']]/td")
        if author_elements:
            author_text = author_elements[0].text_content().strip()
    ...
    description_elements = tree.xpath("//div[@class='summary-text-container']//text()[normalize-space()]")
    description = ""
    if description_elements:
        # Get the first non-empty text content
        for elem in description_elements:
            text = elem.strip()
            if text and not text.startswith("...") and not text.startswith("Read More") and not text.startswith("Show Less"):
                description = text
                break
    ...
    book_id_elements = tree.xpath("//table[@id='about_book_table']//tr[th[text()='EBook-No.']]/td")
    ...
    for row in download_rows:
        ...
        download_urls.append(schemas.DownloadUrlData(
            title=format_title,
            url=download_url
        ))
